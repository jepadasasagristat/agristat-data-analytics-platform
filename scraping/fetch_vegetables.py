"""
Fetch Vegetables and Root Crops Volume of Production + Area Planted/Harvested
from PSA OpenSTAT (PXWeb API), years 2010-2026.

Writes intermediate wide CSVs under data/raw/ for reshape_vegetables.py.

Run from project root:
  python -m scraping.fetch_vegetables
"""
...
# Vegetable tables encode Year as 0=2010, 1=2011, ...
BASE_YEAR = 2010
# Many crop × geo cells; chunk both year and crop to avoid API 403s.
YEAR_CHUNK = 1
CROP_CHUNK = 20
MAX_RETRIES = 4
RETRY_SLEEP_SEC = 2.5
...
def datasets() -> list[Dataset]:
    ...
    return [
        Dataset(
            name="vegetables and root crops volume of production",
            api_url="https://openstat.psa.gov.ph/PXWeb/api/v1/en/DB/2E/CS/0082E4EVCP3.px",
            out_csv=RAW_DIR / "_wide_vegetables_volume.csv",
        ),
        Dataset(
            name="vegetables and root crops area planted/harvested",
            api_url="https://openstat.psa.gov.ph/PXWeb/api/v1/en/DB/2E/CS/0112E4EAHM3.px",
            out_csv=RAW_DIR / "_wide_vegetables_area.csv",
        ),
    ]
...
def value_chunks(values: list[str], size: int) -> list[list[str]]:
...
def build_query(meta: dict, years: list[str], crop_values: list[str] | None = None) -> dict:
    by_code = {v["code"]: v for v in meta["variables"]}
    crops = crop_values if crop_values is not None else by_code["Crop"]["values"]
    return {
        "query": [
            {
                "code": "Crop",
...
def fetch_csv(api_url: str, query: dict) -> str:
    ...
    for attempt in range(1, MAX_RETRIES + 1):
        ...
        try:
            ...
            last_err = err
            if err.code not in (403, 429, 500, 502, 503, 504) or attempt >= MAX_RETRIES:
                raise
...
def fetch_dataset(ds: Dataset) -> None:
    ...
    crop_var = next(v for v in meta["variables"] if v["code"] == "Crop")
    ...
    crop_values = list(crop_var["values"])
    print(f"Fetching {meta['title']}")
    print(f"Years: {YEAR_FROM}-{YEAR_TO} · Crops: {len(crop_values)} (chunks of {CROP_CHUNK})")
    chunks: list[str] = []
    for start, end in year_chunks(YEAR_FROM, YEAR_TO, YEAR_CHUNK):
        codes = year_codes(start, end)
        for batch_i, crop_batch in enumerate(value_chunks(crop_values, CROP_CHUNK), start=1):
            print(
                f"  {start}-{end} crops {batch_i}/{((len(crop_values) - 1) // CROP_CHUNK) + 1} ...",
                end=" ",
                flush=True,
            )
            text = fetch_csv(ds.api_url, build_query(meta, codes, crop_batch))
            chunks.append(text)
            print("ok")
